...
| `configure_elevated_application` | EXE 路径 | `()` | 经一次 UAC 注册固定目标的最高权限计划任务 |
| `remove_elevated_application` | EXE 路径 | `()` | 删除该路径对应的 CouchAxis 计划任务 |
| `launch_application` | EXE 路径、管理员权限开关 | `()` | 普通启动，或运行已注册的最高权限任务 |
